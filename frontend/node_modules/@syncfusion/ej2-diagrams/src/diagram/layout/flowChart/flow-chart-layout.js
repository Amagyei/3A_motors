...
var FlowchartLayout = /** @class */ (function () {
    ...
    FlowchartLayout.prototype.updateVerticalConnectorSegments = function (internalConnector, pts) {
        ...
        if (updatedPts.length > 2) {
            var point_1 = updatedPts[1];
            this.diagram.nodes.forEach(function (node) {
                if (_this.contains(point_1, node.wrapper.bounds)) {
    ...
    FlowchartLayout.prototype.updateLoopConnector = function (internalConnector) {
        ...
        if (this.orientation === 'TopToBottom') {
            var target = Array.from(this.vertexMapper.values()).filter(function (e) {
                return e.item.wrapper.bounds.containsPoint({ x: e.item.wrapper.bounds.x, y: srcNode.offsetY }) &&
                    srcNode.id !== e.item.id;
            });
            var max = Math.max(firstPt.x, lastPt.x);
            var isSiblingsInRight = (target.length > 0 && target[0].item.wrapper.bounds.x > srcBounds.x);
            ...
            var target = Array.from(this.vertexMapper.values()).filter(function (e) {
                return e.item.wrapper.bounds.containsPoint({ x: srcNode.offsetX, y: e.item.wrapper.bounds.y }) &&
                    srcNode.id !== e.item.id;
            });
            var isSiblingsInBottom = (target.length > 0 && target[0].item.wrapper.bounds.y > srcBounds.y);
            var max = Math.max(firstPt.y, lastPt.y);
        ...
        if (edge.annotations && edge.annotations.length) {
            edge.annotations.forEach(function (annotation) {
                if (typeof annotation.content === 'string') {
                    var text_1 = annotation.content.toString();
                    _this.yesBranchValues.forEach(function (branchText) {
                        if (text_1.localeCompare(branchText, undefined, { sensitivity: 'accent' }) === 0) {
                            isYesBranch = true;
                            return;
                        }
                    });
