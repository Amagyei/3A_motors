...
let InterActiveChatBase = class InterActiveChatBase extends Component {
    ...
    renderViewSections(element, headerClassName, viewClassName) {
        const headerWrapper = this.createElement('div', { attrs: { class: headerClassName } });
        element.appendChild(headerWrapper);
        const viewWrapper = this.createElement('div', { attrs: { class: viewClassName } });
        element.appendChild(viewWrapper);
    }
    createViewComponents(viewWrapper) {
        const contentWrapper = this.createElement('div', { attrs: { class: 'e-views' } });
        const viewContainer = this.createElement('div', { attrs: { class: 'e-view-container' } });
    ...
    createSuggestionElement(suggestionHeader) {
        const suggestionContainer = this.createElement('div', { attrs: { class: 'e-suggestions' } });
        const suggestionHeaderElement = this.createElement('div', { attrs: { class: 'e-suggestion-header' } });
        const suggestionListElement = this.createElement('div', { attrs: { class: 'e-suggestion-list' } });
        if (suggestionHeader) {
    ...
    renderBannerView(bannerTemplate, parentElement, templateName) {
        if (bannerTemplate) {
            const introContainer = this.createElement('div', { attrs: { class: 'e-banner-view' } });
    ...
    renderFooterContent(footerTemplate, footer, prompt, promptPlaceholder, showClearButton, rowCount, className) {
        if (footerTemplate) {
            ...
            const textareaEle = this.createElement('textarea', { attrs: { class: className } });
...
let AIAssistView = class AIAssistView extends InterActiveChatBase {
    ...
    renderStopResponding() {
        ...
        const stopRespondingIcon = this.createElement('span', { attrs: { class: 'e-icons e-assist-stop' } });
    ...
    renderOutputContainer(promptText, outputText, index, isMethodCall) {
    ...
    renderOutput(outputContainer, promptText, outputText, isMethodCall, index) {
        const promptIcon = this.createElement('span', { attrs: {
                class: 'e-output-icon e-icons ' + (this.responseIconCss || (this.isAssistView && this.views[0].iconCss) || 'e-assistview-icon')
            } });
        const aiOutputEle = this.createElement('div', { attrs: { class: 'e-output' } });
    ...
    renderOutputTextContainer(response, aiOutputEle, index, isMethodCall) {
        if (this.contentFooterEle) {
            this.contentFooterEle.classList.remove('e-assist-toolbar-active');
        }
    ...
    renderOutputToolbarItems(index) {
    ...
    renderPrompt(promptText, promptIndex) {
        ...
        const promptContent = this.createElement('div', { attrs: { class: 'e-prompt-content' } });
        const promptToolbarContainer = this.createElement('div', { attrs: { class: 'e-prompt-toolbar' } });
        const promptToolbar = this.createElement('div');
        const userIcon = this.createElement('span', { attrs: { class: this.promptIconCss ? 'e-prompt-icon e-icons '
                    + this.promptIconCss : '' } });
    ...
    renderSkeleton() {
        ...
        const outputViewWrapper = this.createElement('div', { attrs: { class: 'e-output', style: 'width: 70%;' } });
        const skeletonIconEle = this.createElement('span', { attrs: { class: 'e-output-icon e-skeleton e-skeleton-text e-shimmer-wave' } });
        const skeletonBodyEle = this.createElement('div', { attrs: { class: 'e-loading-body' } });
        const skeletonFooterEle = this.createElement('div', { attrs: { class: 'e-loading-footer' } });
        ...
        if (!this.isOutputRenderingStop) {
            ...
            if (typeof promptData === 'object') {
                const tPrompt = {
                    ...
                    isResponseHelpful: isNullOrUndefined(promptData.isResponseHelpful) ? null :
...
let ChatUI = class ChatUI extends InterActiveChatBase {
    ...
    renderChatUIView() {
        ...
        if (isNullOrUndefined(this.messages) || this.messages.length <= 0) {
            ...
            this.isEmptyChatTemplateRendered = isNullOrUndefined(this.messageWrapper.querySelector('.e-banner-view')) ? false : true;
        }
        this.wireEvents();
    ...
    renderChatHeaderToolbar(headerContainer) {
        if (!isNullOrUndefined(this.headerToolbar) && this.headerToolbar.items.length > 0) {
            const toolbarEle = this.createElement('div', { attrs: { class: 'e-chat-toolbar' } });
            /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
    ...
    renderMessageGroup(chatContentWrapper) {
        if (this.loadOnDemand) {
            ...
            const initialMessages = this.messages.slice(this.startIndex);
            initialMessages.forEach((msg, i) => {
                const currIndex = this.startIndex + i;
                const prevIndex = i === 0 ? -1 : currIndex - 1;
    ...
    loadLeftGroupOnDemand(msg, loadOldChat, index, messageGroup) {
        ...
        const isAnyMsgPresent = this.messages[index - 1] ? true : false;
        const prevAuthorId = isAnyMsgPresent ? this.messages[index - 1].author.id : '';
    ...
    addGroupItems(msg, messageGroup, isUserTimeStampRendered, showStatus, index, loadOldChat) {
        const messageItem = this.createElement('div', { className: 'e-message-item' });
    ...
    renderNewMessage(msg, index) {
        if (this.isEmptyChatTemplateRendered) {
            const introContainer = this.messageWrapper.querySelector('.e-banner-view');
    ...
    loadMoreMessages() {
        ...
        setTimeout(() => {
            ...
            const endIndex = this.startIndex; // To slice up to the previously rendered message items.
            this.startIndex = Math.max(0, this.startIndex - this.batchSize);
            const nextBatchMessages = this.messages.slice(this.startIndex, endIndex);
            nextBatchMessages.reverse().forEach((msg, i) => {
                const currIndex = (endIndex - i) - 1; // To pass the actual index of the reversed item.
