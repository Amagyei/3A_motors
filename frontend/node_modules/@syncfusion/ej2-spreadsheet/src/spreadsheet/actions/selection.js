    ...
    Selection.prototype.selectMultiRange = function (address, isInit, inRange, skipChecking) {
        ...
        if (this.parent.activeSheetIndex === sheetIdx) {
            address.split(' ').forEach(function (rng, idx) {
                _this.selectRangeByIdx(getRangeIndexes(rng), { type: 'mousedown', ctrlKey: idx !== 0 }, null, inRange, isInit, skipChecking);
            });
        }
        else {
            ...
            if (!(closest(e.target, '.e-findtool-dlg') || closest(e.target, '.e-dragfill-ddb'))) {
                if (this.getSheetElement().contains(e.target) && !e.target.classList.contains('e-colresize')
                    ...
                    if (e.target.classList.contains('e-autofill')) {
                        ...
                        var autoFillDdb = e.target.parentElement &&
                            e.target.parentElement.querySelector('.e-dragfill-ddb');
            ...
            if (isCellReference(str.toUpperCase())) {
                ...
                if (i > 0) {
                    if (parsedVal[i - 1].lastIndexOf('!') === parsedVal[i - 1].length - 1) {
                        ...
                        formulaSheetIdx = getSheetIndex(this.parent, sheetName);
                    }
                }
                if (parsedVal[i + 1] === ':') {
                    i++;
