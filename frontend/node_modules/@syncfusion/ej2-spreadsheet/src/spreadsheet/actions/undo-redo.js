...
import { getRangeFromAddress, getRangeIndexes, workbookEditOperation, clearCFRule, isHeightCheckNeeded } from '../../workbook/index';
...
var UndoRedo = /** @class */ (function () {
    ...
    UndoRedo.prototype.performUndoRedo = function (args) {
        ...
        if (undoRedoArgs) {
            ...
            if (undoRedoArgs.action === 'autofillWithCF') {
                ...
                var conditionalFormat = sheet.conditionalFormats;
            ...
            if (!args.isPublic) {
                ...
                if (this.parent.calculationMode === 'Manual' && (undoRedoArgs.action === 'clipboard' ||
                    ...
                    if ((!args.isUndo && sheet.isSheetCalculated && undoRedoArgs.action !== 'autofill') ||
                        ...
                        for (var i = 0, cellCol = actionData.cutCellDetails.length > 0 ? actionData.cutCellDetails :
                            actionData.cellDetails; i < cellCol.length; i++) {
                            var cellValue = sheet
                                .rows[cellCol[i].rowIndex].cells[cellCol[i].colIndex];
                            cellValue = cellValue ? cellValue.value : '';
                            if (!isNullOrUndefined(cellCol[i].autoFillText)) {
                                cellCol[i].autoFillText = cellValue;
                            }
                            else {
                                if (cellCol[i].value !== cellValue && undoRedoArgs.action !== 'autofill') {
                                    cellCol[i].value = cellValue;
                                }
                            }
                        }
                    }
                        ...
                        if (undoActionName) {
                            for (var i = 0, copyCellCol = actionData.cellDetails, cutCellCol = actionData.cutCellDetails; i < copyCellCol.length; i++) {
                                var cellValue = sheet
                                    .rows[copyCellCol[i].rowIndex].cells[copyCellCol[i].colIndex];
                                cellValue = cellValue ? cellValue.value : '';
                                if (undoActionName === 'Copy') {
                                    copyCellCol[i].copyCellValue = cellValue;
                                }
                ...
                if (args.isUndo) {
                    ...
                    if (undoRedoArgs.action === 'cellSave' && this.parent.calculationMode === 'Manual' &&
                        ...
                        var cell = getCell(actionData.cellDetails[0].rowIndex, actionData.cellDetails[0].colIndex, sheet);
    ...
    UndoRedo.prototype.updateCellDetails = function (cells, sheet, range, isRefresh, args, preventEvt, isColSelected, isUndoRedo, isFromAutoFillOption) {
        ...
        if (isRefresh) {
            ...
            this.parent.serviceLocator.getService('cell').refreshRange(range, false, false, true, false, isImported(this.parent), null, isFromAutoFillOption, isHeightCheckFromUndo);
