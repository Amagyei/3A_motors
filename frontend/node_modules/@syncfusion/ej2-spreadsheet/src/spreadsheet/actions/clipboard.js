...
import { getRangeIndexes, getCell, getSheet, getSwapRange, inRange, getCellAddress, isReadOnly, getRow } from '../../workbook/index';
...
import { ribbonClick, copy, paste, initiateFilterUI, setPosition, isLockedCells, focus, isReadOnlyCells, readonlyAlert } from '../common/index';
                            ...
                            for (var x = selIdx[0]; x <= selIdx[2]; x += (cIdx[2] - cIdx[0]) + 1) {
                                ...
                                for (var y = selIdx[1]; y <= selIdx[3]; y += (cIdx[3] - cIdx[1] + 1)) {
                                    ...
                                    if (this.copiedInfo && !this.copiedInfo.isCut && cell.validation) {
                                        var currIdx = selIdx;
                                        var prevIdx = cIdx;
                                        var updatedVal = getUpdatedFormula(currIdx, prevIdx, prevSheet, this.parent, { formula: cell.validation.value1 });
            ...
            if (!cancel && cell) {
                ...
                if (cell.style && args.isExternal) {
                    var hgt = getTextHeightWithBorder(_this.parent, rIdx, cIdx, sheet, cell.style || _this.parent.cellStyle, cell.wrap ?
                        getLines(_this.parent.getDisplayText(cell), getExcludedColumnWidth(sheet, rIdx, cIdx, cell.colSpan > 1 ? cIdx + cell.colSpan - 1 : cIdx), cell.style, _this.parent.cellStyle) : 1);
                    hgt = Math.round(hgt);
                    if (hgt < 20) {
                    ...
                    if (col || cellStyle_1) {
                        ...
                        if (col) {
                            if (checkIsFormula(col)) {
                                ...
                                cells_1[j].value = parseIntValue(col.trim(), true);
                            }
                        }
                    }
                });
    ...
    Clipboard.prototype.generateCells = function (ele, pasteModelArgs) {
        ...
        for (var rowIdx = 0, rowLen = pasteModelArgs.usedRowIndex; rowIdx <= rowLen; rowIdx++) {
            ...
            for (var colIdx = 0; colIdx <= colLen; colIdx++) {
                ...
                if (td.textContent) {
                    cells[colIdx].value = parseIntValue(td.textContent.trim(), true);
                }
                ...
                if (td.getAttribute(formatStr)) {
                    ...
                    if (cells[colIdx].value && td.getAttribute('cell-value')) {
                        cells[colIdx].value = parseIntValue(td.getAttribute('cell-value').trim(), true);
                    }
                }
