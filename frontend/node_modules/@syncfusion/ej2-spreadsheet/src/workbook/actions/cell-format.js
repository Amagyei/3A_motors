import { getRangeIndexes, setCellFormat, applyCellFormat, activeCellChanged, isReadOnly } from '../common/index';
...
var WorkbookCellFormat = /** @class */ (function () {
    ...
    WorkbookCellFormat.prototype.format = function (args) {
        ...
        var isReadonlyCell = isReadOnly(getCell(indexes[0], indexes[1], sheet), getColumn(sheet, indexes[1]), getRow(sheet, indexes[0]));
        var isReadonlyColumn = getColumn(sheet, indexes[1]) ? getColumn(sheet, indexes[1]).isReadOnly : false;
        var isReadonlyRow = getRow(sheet, indexes[0]) ? getRow(sheet, indexes[0]).isReadOnly : false;
        ...
        if (Object.keys(eventArgs.style).length) {
            ...
            for (i = indexes[0]; i <= indexes[2]; i++) {
                ...
                for (j = indexes[1]; j <= indexes[3]; j++) {
                    ...
                    var readonlyColumn = getColumn(sheet, j);
                    var readOnlyRow = getRow(sheet, i);
